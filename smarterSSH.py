...
"""
 *  PeerMon
 *  Copyright 2011 Tia Newhall, Janis Libeks, Ross Greenwood, Jeff Knerr, Steve Dini
 *
...
MEM_DELTA=1000000     # 1 GB differences are not significant for ordering hosts
...
class SmarterSSH():
  ...
  def SSH(self):
    """
    Perform the actual SSH
    """
    random.shuffle(self.data)
    if not self.rand:
      self._sort() #sort results based on either CPU load, or FreeMem or both
    program = "ssh"
    try:
      target = self.data[0].HOSTNAME
    except:
      print "Found no nodes to connect to"
      raise SystemExit
    arg = [target]
    print "SSHing into ", target
    try:
...
#======================================================================
def main():
  descriptiontext = "SmarterSSH. Version 2.0"
